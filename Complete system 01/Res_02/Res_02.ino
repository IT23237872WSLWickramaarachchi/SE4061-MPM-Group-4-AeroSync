...
typedef struct struct_message {
  int val; // Brightness level (0 - 100)
} struct_message;
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  int brightness = constrain(myData.val, 0, 100);
  Serial.printf("Brightness Received: %d%%\n", brightness);
  ...
  } else if (brightness >= 20 && brightness <= 70) {
...
void setup() {
  ...
  Serial.println("Brightness Receiver Ready.");
}
